...
import { Copy, MoreVertical, Pin, PinOff, RefreshCcw, Share2, Star, Trash2 } from "lucide-react"
...
export default function DashboardPage() {
    ...
    }, [isInitialized, userData, filter])

    const fetchFeedbacks = async (userId: string) => {
        try {
            setLoading(true)
            const res = await axios.post("/api/get-feedbacks", { userId, filter })
            if (res.data.success) {
    ...
    const toggleLike = async (feedbackId: string) => {
        try {
            setLikeLoading(feedbackId)
            const res = await axios.get(`/api/like-feedback?feedbackId=${feedbackId}`)
            if (res.data.success) {
                setFeedbacks((prev) => prev.map((f) =>
                    f._id === feedbackId ? {
                        ...f,
                        like: res.data.feedback.like
                    } : f
                ))
        ...
        <main className="min-h-[90vh] w-full px-4 sm:px-6 md:px-10 py-10 bg-slate-50 dark:bg-neutral-950 text-black dark:text-white">
            <div className="max-w-5xl mx-auto w-full flex flex-col gap-6">
                ...
                <section className="mt-10">
                    ...
                    {loading ? (
                        // Show Loading until the feedbacks are fetched
                        <div className="flex justify-center py-10">
                            <Loader2 className="w-6 h-6 animate-spin" />
                        </div>
                    ) : feedbacks.length === 0 ? (
                        // Show No feedbacks message if no feedbacks are found
                        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
                            {
                                filter === "favorite" ?
                                    "No favorite feedbacks yet."
                                    :
                                    "No feedbacks yet. Share your link to get responses."
                            }
                        </p>
                    ) : (
                        // Show Feedbacks
                            ...
                            {feedbacks.map((feed) => (
                                ...
                                >
                                    <div className="absolute top-2 right-2 flex items-center gap-1">
                                        ...
                                        {
                                            // Show Loading until the feedback is Deleted
